...
//		return str.replace("-", File.separator);
	...
	public String uploadFile(String uploadFolder, String dateFolder, MultipartFile uploadFile) {
		...
		// 업로드 할 폴더 file객체.  c:/dev/upload/pds   2025/05/02
		File file = new File(uploadFolder, dateFolder);  // 예> "C:\\Dev\\upload\\pds"  "2024\11\18"
		
		// "C:\\Dev\\upload\\pds\\2024\\11\\18" 폴더
		if(file.exists() == false) {
			file.mkdirs(); // 하위폴더까지 폴더생성 "C:\\Dev\\upload\\pds\\2024\\11\\18"    c:/dev/upload/pds/2025/05/02
		}
		...
		// 파일업로드작업
		try {
			File saveFile = new File(file, realUploadFileName);  // c:/dev/upload/pds/2025/05/02   2f48f241-9d64-4d16-bf56-70b9d4e0e79a_abc.gif
			uploadFile.transferTo(saveFile); // 파일복사..  파일업로드
			
			// 썸네일 작업을 위한 이미지 파일체크.
			if(checkImageType(saveFile)) { // 이미지파일
				
				// 썸네일작업을 위한 File객체.  pom.xml파일에 썸네일 기능을 제공하는 의존성구문.
